...
async def _run_hook(end: bool) -> str:
    guardrail = _BlockingGuardrail(
        guardrail_name="test-blocking-guardrail", event_hook="post_call"
    )
    # Process every chunk so the per-chunk block path triggers when end=False.
...
def _assert_clean_block_termination(raw: str) -> None:
    ...
    assert any(
        '"stop_reason"' in block and "message_delta" in block
        for block in raw.split("\n\n")
    )
